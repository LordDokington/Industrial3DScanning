...
class Matrix
{
  public:
    Matrix():m_M(0), m_N(0){}
    Matrix(size_t m, size_t n): m_M(m), m_N(n){ m_data.resize(m*n); }

    void resize(size_t m, size_t n) { m_data.resize(m*n); m_M=m; m_N=n;}
  ...
  private:
    ...
    QVector<double> m_data; ///< storage for the matrix elements
};
